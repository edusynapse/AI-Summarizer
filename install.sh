...
# summaries/prompt_template.txt, summaries/README.md, repo_context/README.md,
# .gitignores, top-level READMEs) are always overwritten.
#
# Per-repo files (repo/config.json, summaries/config.json, and any hand-curated
# repo_context/*.md beyond the starter README) are NEVER overwritten if they
# already exist — only created on first install.
#
# Generated artifacts (repo/index.sqlite, repo/summaries/, summaries/manifest.json,
# summaries/repo/) are never touched.
...
chmod +x "$DST_DIR/broker.py" "$DST_DIR/indexer.py" "$DST_DIR/summaries/summarizer.py" || true
